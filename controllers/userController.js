...
module.exports.deleteUserById= async (req,res) => {
    try {
        ...
        await ProgrammeDeVoyageModel.updateMany({owner : id},{
            $unset: { owner: 1 },// null "" 
          });
...
module.exports.login= async (req,res) => {
    try {
        ...
        res.cookie("jwt_token_randa", token, {httpOnly:false,maxAge:maxTime * 1000})
        res.status(200).json({user})
    } catch (error) {
        res.status(500).json({message: error.message});
    }
...
module.exports.logout= async (req,res) => {
    try {
        ...
        res.cookie("jwt_token_randa", "", {httpOnly:false,maxAge:1})
